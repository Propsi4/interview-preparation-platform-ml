...
async def chat_with_technical_interview_agent(
    ...
    try:
        ...
        logger.error(f"Error running technical interview for session {session_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to run technical interview")
